...
class RuleLoader:
    ...
    def get_packaging_rules(self):
        """Load packaging patterns. Scope: CATEGORY."""
    ...
    def get_sparkling_rules(self):
        """Load sparkling/still patterns. Scope: CATEGORY."""
    ...
    def get_product_type_rules(self):
        """Load product type patterns. Scope: CATEGORY."""
        rules = self._load_category_patterns("product_types.json")
        return [(pattern, entry["type"]) for pattern, entry in rules]

    def get_stop_words(self):
        """Load stop phrases and words. Scope: CATEGORY.
    ...
    def get_flavor_rules(self):
        """Load flavor patterns. Scope: CATEGORY.
    ...
    def get_sanity_checker(self):
        """Load sanity check function. Scope: CATEGORY.

        Convention: category dir may contain sanity.py with sanity_check_size().
        """
        dir_name = _CATEGORY_DIR_MAP.get(self.category_slug)
        if not dir_name:
            return None
        try:
            module = importlib.import_module(
                f"normalizer.rules.category.{dir_name}.sanity"
            )
            return getattr(module, "sanity_check_size", None)
        except ImportError:
            return None

    def get_inference_fn(self):
        """Load post-parse inference function. Scope: CATEGORY.

        Convention: category dir may contain inference.py with apply_inference().
        """
        dir_name = _CATEGORY_DIR_MAP.get(self.category_slug)
        if not dir_name:
            return None
        try:
            module = importlib.import_module(
                f"normalizer.rules.category.{dir_name}.inference"
            )
            return getattr(module, "apply_inference", None)
        except ImportError:
            return None
    ...
    def get_category_mapping_path(self, source_type):
        ...
        Prefers category-specific file (e.g., wolt_spirtsiz-ickiler_categories.json)
        over generic file (e.g., wolt_categories.json).
        """
        if not self.market:
            raise ValueError("Market not set on RuleLoader")
        market_dir = _RULES_DIR / "market" / self.market
        # Prefer category-specific mapping file
        if self.category_slug:
            cat_path = market_dir / f"{source_type}_{self.category_slug}_categories.json"
            if cat_path.exists():
                return cat_path
        # Fall back to generic mapping file
        path = market_dir / f"{source_type}_categories.json"
        if not path.exists():
